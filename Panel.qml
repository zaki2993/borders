  ...
  KeyboardPanel {
    ...
    PanelKeyCatcher { id: catcher; anchors.fill: parent; onCloseRequested: root.close(); onTabRequested: function(direction) { root.switchPanel(direction) }
      Column { id: body; width: parent.width; spacing: Style.space(9)
        ...
        Toggle { width: parent.width; label: "Shining gradient borders"; description: "Blend three colors around the active window."; checked: root.service ? root.service.shine : false; foreground: root.foreground; fontFamily: root.fontFamily; onClicked: root.service.setShine(!checked) }
